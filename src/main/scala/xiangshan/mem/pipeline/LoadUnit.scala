...
import xiangshan.mem.prefetch.L1PrefetchReq
...
class LoadUnit_S0(implicit p: Parameters) extends XSModule with HasDCacheParameters{
  ...
  val s0_mask = WireInit(0.U(8.W))
  val s0_uop = WireInit(io.in.bits.uop)
  val s0_isFirstIssue = WireInit(io.isFirstIssue)
...
//  val s0_intloadFirstIssue_valid = WireInit(io.in.valid && io.isFirstIssue)
  ...
  val s0_isSoftPrefetch = WireInit(false.B)
  val s0_isSoftPrefetchRead = WireInit(false.B)
  val s0_isSoftPrefetchWrite = WireInit(false.B)
  val s0_ishwPrefetchRead = WireInit(false.B)
  val s0_isPrefetch = s0_isSoftPrefetch||s0_ishwPrefetchRead

  val tryFastpath = WireInit(false.B)
  val s0_valid = s0_intloadIssue_valid || s0_hw_pf_select

  when(s0_hw_pf_select) {
    s0_vaddr := io.prefetchReq.bits.getVaddr()
    s0_ishwPrefetchRead := true.B
  }.otherwise{
    ...
    s0_isSoftPrefetch := LSUOpType.isPrefetch(s0_uop.ctrl.fuOpType)
    s0_isSoftPrefetchRead := s0_uop.ctrl.fuOpType === LSUOpType.prefetch_r
    s0_isSoftPrefetchWrite := s0_uop.ctrl.fuOpType === LSUOpType.prefetch_w
  }
  ...
  // query DTLB
  io.dtlbReq.valid := s0_valid || tryFastpath
  io.dtlbReq.bits.vaddr := s0_vaddr
  // intloadFirstIssue and HWprefetch are TlbCmd.read
  io.dtlbReq.bits.cmd := Mux(s0_isSoftPrefetch, Mux(s0_isSoftPrefetchWrite,TlbCmd.write, TlbCmd.read), TlbCmd.read)
  ...
  // query DCache
  io.dcacheReq.valid := s0_valid || tryFastpath
  when (s0_ishwPrefetchRead) {
    io.dcacheReq.bits.cmd  := MemoryOpConstants.M_PFR
  }.elsewhen (s0_isSoftPrefetchRead) {
    io.dcacheReq.bits.cmd  := MemoryOpConstants.M_PFR
  ...
  io.out.valid := (s0_valid || tryFastpath)
  ...
  io.out.bits.isSoftPrefetch := s0_isSoftPrefetch
  io.out.bits.ishwPrefetch := s0_ishwPrefetchRead
  io.in.ready := (!s0_valid) || io.out.ready
  ...
  XSPerfAccumulate("in_fire_first_issue", s0_valid && io.isFirstIssue)
...
class LoadUnit_S1(implicit p: Parameters) extends XSModule {
  ...
  val s1_is_prefetch = io.in.bits.ishwPrefetch
  val s1_is_hw_prefetch = s1_is_prefetch && io.in.bits.ishwPrefetch
  ...
  io.rsFeedback.valid := io.in.valid && (s1_bank_conflict || needLdVioCheckRedo || io.s1_cancel) && !io.s1_kill && !s1_is_hw_prefetch
  ...
  io.out.valid := io.in.valid && !io.s1_kill && (!io.rsFeedback.valid || (!needLdVioCheckRedo && !s1_bank_conflict || s1_is_prefetch))
...
class LoadUnit_S2(implicit p: Parameters) extends XSModule with HasLoadHelper {
  ...
  val s2_is_prefetch = io.in.bits.isSoftPrefetch || io.in.bits.ishwPrefetch
  val s2_is_hwPrefetch = io.in.bits.ishwPrefetch
  ...
  io.out.valid := io.in.valid &&
    ...
    !s2_data_invalid &&
    !s2_is_hwPrefetch // hardware prefetch flow should not be writebacked
...
class LoadUnit(implicit p: Parameters) extends XSModule with HasLoadHelper with HasPerfEvents with SdtrigExt {
  ...
  load_s0.io.prefetchReq <> io.prefetch_req
  ...
  coreParams.l1dprefetchRefill.map(_ => {
    io.prefetch_train.bits.meta_prefetch := io.dcache.resp.bits.meta_prefetch.get
    io.prefetch_train.bits.meta_access := io.dcache.resp.bits.meta_access.get
  })
