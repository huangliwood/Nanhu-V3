...
VERBOSE=1
LOGGED=0
LOG_PATH="$PRO_PATH/logs/$(date '+%m-%d')/$OUTPUT_FILE"
#EMU_Parameters
...
fun_openVerdi(){
  echo -e "${PURPLE}------------------------------------------------\n*${FUNCNAME}*${NC}"
    ...
    if [[ -z $op_tlTest ]];then
      ...
      [[ $(find ./ -name firrtl_black_box_resource_files.f | xargs sed -n '/SimTop/p') == "" ]] && realpath ./SimTop.v >> firrtl_black_box_resource_files.f
